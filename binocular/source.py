...
class C_Code:
    ...
    def normalize(func_def: tree_sitter.Node, encoding: str = "utf8"):
        ...
        if fptr_ret:
            ...
            param_list = traverse(next_n, "function_declarator", "parameter_list")
        else:
        ...
        for param_dec in [
            node for node in param_list.children if node.type == "parameter_declaration"
        ]:
            # Argument Name
            ...
            if param_func_dec is not None:
                ...
                fptr_params = str(
                    get_child_by_type(param_func_dec, "parameter_list").text,
                    encoding=encoding,
                )
                arguments.append(
                    dict(
            ...
            # This check is probably unneeded because of the above variadic_parameter
                ...
                arguments.append(dict(data_type=data_type, var_name=param_name))
        ...
        return dict(
            ...
            name=name,
            source=str(
                get_child_by_type(func_def, "compound_statement").text,
                encoding=encoding,
            ),
            argv=arguments,
